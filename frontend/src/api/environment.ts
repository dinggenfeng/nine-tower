...
export async function createEnvironment(
  ...
  const res = await request.post<Environment>(`/projects/${projectId}/environments`, data);
  return res.data;
}

export async function listEnvironments(projectId: number): Promise<Environment[]> {
  const res = await request.get<Environment[]>(`/projects/${projectId}/environments`);
  return res.data;
}
...
export async function addConfig(envId: number, data: EnvConfigRequest): Promise<EnvConfig> {
  const res = await request.post<EnvConfig>(`/environments/${envId}/configs`, data);
  return res.data;
}
...
export async function updateConfig(configId: number, data: EnvConfigRequest): Promise<EnvConfig> {
